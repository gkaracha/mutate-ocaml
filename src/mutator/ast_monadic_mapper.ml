...
(* Some utilities *)
let map_fst f (x, y) = Monad.(f x >>= fun z -> return (z, y))
let map_snd f (x, y) = Monad.(f y >>= fun z -> return (x, z))
...
module C = struct
  ...
  let map sub c =
    ...
    | Pconst_string (s, loc, quotation_delimiter) ->
        sub.location sub loc >>= fun loc ->
        return (Const.string ~loc ?quotation_delimiter s)
end
...
module T = struct
  ...
  let row_field sub {
    ...
    let open Monad in
    sub.location sub prf_loc >>= fun loc ->
    sub.attributes sub prf_attributes >>= fun attrs ->
    (match prf_desc with
      | Rtag (l, b, tl) ->
        map_loc sub l         >>= fun l ->
        mmap (sub.typ sub) tl >>= fun tl ->
        return (Rtag (l, b, tl))
      | Rinherit t ->
        sub.typ sub t >>= fun t ->
        return (Rinherit t)
    ) >>= fun desc ->
    return (Rf.mk ~loc ~attrs desc)
  ...
  let object_field sub {
    ...
    let open Monad in
    sub.location sub pof_loc          >>= fun loc ->
    sub.attributes sub pof_attributes >>= fun attrs ->
    (match pof_desc with
      | Otag (l, t) ->
        map_loc sub l >>= fun l ->
        sub.typ sub t >>= fun t ->
  ...
  let map sub {ptyp_desc = desc; ptyp_loc = loc; ptyp_attributes = attrs; ptyp_loc_stack = _} =
    ...
    match desc with
    | Ptyp_any -> return (any ~loc ~attrs ())
    | Ptyp_var s -> return (var ~loc ~attrs s)
    | Ptyp_arrow (lab, t1, t2) ->
        arrow ~loc ~attrs lab <$> (sub.typ sub t1) <*> (sub.typ sub t2)
    | Ptyp_tuple tyl -> tuple ~loc ~attrs <$> mmap (sub.typ sub) tyl
    | Ptyp_constr (lid, tl) ->
        constr ~loc ~attrs <$> (map_loc sub lid) <*> (mmap (sub.typ sub) tl)
    | Ptyp_object (l, o) ->
        object_ ~loc ~attrs <$> (mmap (object_field sub) l) <*> return o
    | Ptyp_class (lid, tl) ->
        class_ ~loc ~attrs <$> (map_loc sub lid) <*> (mmap (sub.typ sub) tl)
    | Ptyp_alias (t, s) -> alias ~loc ~attrs <$> (sub.typ sub t) <*> return s
    | Ptyp_variant (rl, b, ll) ->
        variant ~loc ~attrs <$> (mmap (row_field sub) rl) <*> return b <*> return ll
    | Ptyp_poly (sl, t) -> poly ~loc ~attrs <$> (mmap (map_loc sub) sl) <*> (sub.typ sub t)
    | Ptyp_package (lid, l) ->
        package ~loc ~attrs <$> (map_loc sub lid) <*> (mmap (map_tuple (map_loc sub) (sub.typ sub)) l)
